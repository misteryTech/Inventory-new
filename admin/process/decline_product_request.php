...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['decline_request'])) {
    ...
    if ($request_id > 0 && !empty($batch_number)) {
        ...
        $query = "
            ...
            SET status = 'Declined', approved1 = '$session_user' 
            WHERE request_id = $request_id AND product_id = '$batch_number'
        ";

        if (mysqli_query($conn, $query)) {
            // Redirect back to the request page
            header("Location: ../view-requestpage.php?request_id=$request_id&username=" . urlencode($session_user) . "&success=Product Decline successfully!");
            exit;
        } else {
